...
//check readme
#define H_ACTIVE 655    // (active + frontporch - 1) 640+16-1 = 655
#define V_ACTIVE 479    // (active - 1)  480 - 1 = 479 \
                        // #define RGB_ACTIVE 319  // (horizontal active)/2 - 1.
#define RGB_ACTIVE 639  // change to this if 1 pixel/byte

// Length of the pixel array, and number of DMA transfers
const unsigned int pixels = screenWidth * screenHeight;
#define TXCOUNT pixels          // Total pixels / 2. The total size of the buffer
...
const int QVGALastLine = 480;                                        //420 / 2      //La cantidad de lineas de los gráficos. Se usa para pasar la info al PIO (si se cambia se mueve la imagen)
unsigned char vga_data_array[TXCOUNT];                               // Pixel color array that is DMA's to the PIO machines and
volatile unsigned char* address_pointer_array = &vga_data_array[0];  // a pointer to the ADDRESS of this color array.
unsigned char vga_data_array_next[TXCOUNT];                          // Note that this array is automatically initialized to all 0's (black)
...
void __not_in_flash_func(QVgaLine)() {
  ...
  if (currentScanLine >= screenWidth) {  // last scanline?
...
void initVGA() {
  ...
  // pio_set_sm_mask_enabled(pio, hsync_sm, true);
  // pio_set_sm_mask_enabled(pio, vsync_sm, true);
  pio_set_sm_mask_enabled(pio, rgb_sm, true);
...
// void hsync_off() {
//   PIO pio = pio0;  // pio_enable_sm_mask_in_sync(pio, ((1u << hsync_sm) | (1u << vsync_sm));
//   uint hsync_sm = 0;
//   uint vsync_sm = 1;
//   uint rgb_sm = 2;
//   uint hsync_offset = pio_add_program(pio, &hsync_program);
//   //hsync_drift(pio, hsync_sm, hsync_offset, HSYNC);
//   pio_sm_put_blocking(pio, hsync_sm, currentScanLine - 400);
...
void hsync_drift(float drift_divide) {
  PIO pio = pio0;
  uint vsync_sm = 1;
  uint hsync_sm = 0;
  uint hdrift_sm = 3;
  pio_sm_set_clkdiv(pio, hsync_sm, drift_divide);
  // pio_sm_set_clkdiv(pio, vsync_sm, drift_divide);

  //   // uint vsync_offset = pio_add_program(pio, &vsync_program);
  //   // uint hdrift_offset = pio_add_program(pio, &hdrift_program);
  //   pio_sm_put_blocking(pio, hsync_sm, H_ACTIVE);
  //   hsync_program_init(pio, hsync_sm, hsync_offset, HSYNC, drift_divide);
  //   //  pio_set_sm_mask_enabled(pio, vsync_sm, true);
  //   hsync_program_init(pio, hsync_sm, hsync_offset, HSYNC, divide);
  //  hsync_program_disable(pio, hsync_sm, hsync_offset, HSYNC, drift_divide);
  //   //  pio_set_sm_mask_enabled(pio, vsync_sm, true);
  // uint hsync_sm = 0;
  //pio_sm_put_blocking(pi, hsync_sm, H_ACTIVE);
  // hsync_program_init(pio, hsync_sm, hsync_offset, HSYNC, drift_divide);
  // pio_enable_sm_mask_in_sync(pio, ((1u << hsync_sm) | (1u << vsync_sm)));
  // pio_enable_sm_mask_in_sync(pio, ((1u << hdrift_sm) | (1u << vsync_sm)));
  // pio_sm_set_enabled(pio, hsync_sm, true);
  // pio_sm_set_enabled(pio, hdrift_sm, true);
}
void hsync_return(float divide) {
  PIO pio = pio0;
  uint vsync_sm = 1;
  uint hsync_sm = 0;
  uint hdrift_sm = 3;
  pio_sm_set_clkdiv(pio, hsync_sm, divide);
  // pio_sm_set_clkdiv(pio, vsync_sm, divide);
}
// void set_vsync() {
//   PIO pio = pio0;  // pio_enable_sm_mask_in_sync(pio, ((1u << hsync_sm) | (1u << vsync_sm));
//   uint vsync_sm = 1;
//   pio_sm_set_enabled(pio, vsync_sm, false);
// }
