...
// // int read() Reads a single sample of recorded ADC data, as a 16-bit value.
// // When multiple pins are recorded the first read will be pin 0, the second will be pin 1, etc. Applications need to keep track of which pin is being returned (normally by always reading out all pins at once). Will not return until data is available.
// // int available() Returns the number of samples that can be read without potentially blocking.
// // void onReceive(void (*fn)(void)) Sets a callback to be called when a-
// // ADC input DMA buffer is fully filled.
// // Will be in an interrupt context so the specified function must
// // operate quickly and not use blocking calls like delay().
...
// ADCInput adc_r(ADC_RED_PIN);
// ADCInput adc_g(ADC_GREEN_PIN);
// ADCInput adc_b(ADC_BLUE_PIN);
...
//   bool setPins(pin_size_t pin [, pin1, pin2, pin3])
//   Adjusts the pin to record. Only legal before ADCInput::begin().
//   ADCInput.setPins(adc_r, adc_g, adc_b);

//   adc_r.setFrequency(1000);
//   adc_g.setFrequency(1000);
//   adc_b.setFrequency(1000);
...
//   void onReceive(void (*fn)(void)) Sets a callback to be called when a-
//   adc_r.onReceive(red_callback);
//     adc_g.onReceive(green_callback);
//       adc_b.onReceive(blue_callback);
// }
// // while (1) {
// //   Serial.printf("%d\n", mike.read());
